...
export async function generateMetadata({ params: { locale } }: { params: { locale: string } }) {
  ...
  const description = about.description || 'Learn more about me, my work, and my skills.';
...
export default function About({ params: { locale } }: { params: { locale: string } }) {
  ...
  /** Build Table of Contents structure dynamically */
  const structure = [
    {
...
{about.openSource.display && (
  <>
    <Heading
    as="h2" id={about.openSource.title} variant="display-strong-s" marginBottom="40">
         {about.openSource.title}
    </Heading>

    <Flex direction="column" fillWidth gap="l">
      {about.openSource.contributions.map((item, idx) => (
       <Flex key={idx} fillWidth gap="4" direction="column">
          <Text variant="heading-strong-l">{item.project}</Text>
          <Text variant="body-default-m" onBackground="neutral-weak">
            {item.description}
          </Text>
          <ul>
            {item.prs.map((pr, prIdx) => (
              <li key={prIdx}>
                <a href={pr.url} target="_blank" rel="noopener noreferrer">{pr.label}</a>
              </li>
            ))}
          </ul>
        </Flex>
